...
from mlcl.pgm.module.supervised_module import PgmSupervisedModule


class PgmAuxiliaryModule(PgmSupervisedModule):
    def __init__(
            self,
            cfg: DictConfig,
        ...
        super(PgmAuxiliaryModule, self).__init__(cfg, model)
    ...
    def _step(self, split: str, batch, batch_idx: int) -> torch.Tensor:
        ...
        self.log_target_metrics(split, y, y_hat, supervised_loss)
        self.log_auxiliary_metrics(split, rules, rules_hat, auxiliary_loss)
    ...
    def log_auxiliary_metrics(self, split: str, rules: torch.Tensor, rules_hat: torch.Tensor, loss: torch.Tensor):
