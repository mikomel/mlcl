...
from mlcl.pgm.module.auxiliary_module import PgmAuxiliaryModule


class PgmMLCLModule(PgmAuxiliaryModule, ThreePhaseTrainableModule):
    """
    Multi-label Contrastive Learning (MLCL) module for PGM-style datasets.
    ...
    def __init__(
        ...
        super(PgmMLCLModule, self).__init__(
    ...
    def _step(self, split: str, batch, batch_idx: int) -> torch.Tensor:
        ...
        self.log_target_metrics(split, duplicated_y, y_hat, supervised_loss)
        self.log_auxiliary_metrics(split, duplicated_rules, rules_hat, auxiliary_loss)
        self.log_contrastive_metrics(split, contrastive_loss)
    ...
    def log_contrastive_metrics(self, split: str, loss: torch.Tensor):
