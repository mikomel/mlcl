...
class Augmentor:
    ...
    def transform_rpm(self, images: np.array, rules: np.array) -> np.array:
        ...
        :param rules: numpy array representing rules of RPM with shape (8, 9)
        :return: numpy array with augmented RPM images with shape (16, width, height)
        """
        kwargs = {f"image{i + 1}": images[i + 1, :, :] for i in range(15)}
        transform = self.position_preserving_transform if self._has_progression_or_arithmetic_on_position(rules) else self.transform
    ...
    def _has_progression_or_arithmetic_on_position(self, rules: np.array) -> bool:
        """ Checks if given RPM has progression (1) or arithmetic (2) relation applied to position. """
        for r in rules:
            if (r[1] and r[5]) or (r[2] and r[5]):
                return True
        return False
